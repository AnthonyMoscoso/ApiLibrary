...
Constraint Fk_Reservation_IdEmployee foreign key (IdEmployee) references Employee (IdEmployee),
Constraint Fk_Reservation_IdBuyer foreign key (IdBuyer) references Person (IdPerson)
);


create table ReservationLine (
IdReservationLine varchar (40) not null primary key ,
IdReservation varchar (40) not null ,
...
LastUpdateDate datetime not null, -- 12/05/2021 12:45:00
StatusCode int not null, -- 1 
Constraint Fk_ReservationLine_IdReservation foreign key (IdReservation) references Reservation (IdReservation),
Constraint Fk_ReservationLine_IdEmployee foreign key (IdBook) references Book (IdBook)
);
